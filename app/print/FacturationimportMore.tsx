...
import { BACKEND_URL, formatNombre } from "@/lib/utils";
...
const FacturationimportMore = ({
  factures,
  rubriquesFact,
}: {
  factures?: IFactureImport[];
  rubriquesFact?: IRubriqueFacture[];
}) => {
  // const clientRubrique = rubriques.find((cl) => cl.client == facture?.journalType?.Client?.nom_client);
  const rubriquesFromBdd = rubriquesFact;

  return (
    <PDFViewer height={500} width={1200}>
      <Document>
        {factures?.map((facture: IFactureImport | any, i) =>
          ...
          new Date(moment("2025-08-02").format("YYYY-MM-DD")) ? (
            <Page key={i} size={"A4"} style={styles.page}>
              ...
              <View style={styles.sceauContainer}>
                <Image
                  src={`${BACKEND_URL}sceau-afritac.png`}
                  style={styles.sceau}
                />
                <Image
                  src={`${BACKEND_URL}paraphe-tdl.png`}
                  style={styles.paraphe}
                />
              </View>
            </Page>
          ) : (
            ...
            <Page key={i} size={"A4"} style={styles.page}>
              ...
              >
                ...
                >
                  ...
                  >
                    {/* <Text>{facture?.detailFacture[0]?.contenu?.map()}</Text> */}
                    {facture?.detailFacture[0]?.contenu?.map(
                      (detail: any, i2: any) => (
                        <View
                          key={i2}
                          style={{ display: "flex", flexDirection: "row" }}
                        >
                          ...
                          >
                            {
                              rubriquesFromBdd?.find((rf) => (rf?.id == detail?.rubrique))?.libelle}
                            {/* {detail?.rubrique} */}
                          </Text>
                          <Text
                            style={{
                  ...
                  <View style={{ width: "50%", fontSize: 8 }}>
                    ...
                    >
                      ...
                      >
                        ...
                        <Text style={{ textAlign: "right", width: "50%" }}>
                            $ {formatNombre(facture?.detailFacture[0]?.contenu?.reduce((init:any,acc:any)=>{return init+((parseFloat(acc?.qt))*(parseFloat(acc?.prix)))},0))}
                        </Text>
                      </View>
                      <View
                        style={{
                      ...
                      >
                        ...
                        >
                          TVA 0% de $ {formatNombre(facture?.detailFacture[0]?.contenu?.reduce((init:any,acc:any)=>{return init+((parseFloat(acc?.qt))*(parseFloat(acc?.prix)))},0))}
                        </Text>
                        <Text
                          style={{
                            width: "40%",
                        ...
                        >
                          $ {formatNombre(((facture?.detailFacture[0]?.contenu?.reduce((init:any,acc:any)=>{return init+((parseFloat(acc?.qt))*(parseFloat(acc?.prix)))},0))*0)/100)}                          
                        </Text>
                      </View>
                      <View
                        style={{
                      ...
                      >
                        ...
                        >
                          TVA 16% de $ {
                                        formatNombre(facture?.detailFacture[0]?.contenu?.
                                          filter((d:any)=>{return d?.exonereTva=="N"})?.
                      ...
                      >
                        ...
                        >
                         $ 
                          ...
                          formatNombre(facture?.detailFacture[0]?.contenu?.reduce((init:any,acc:any)=>{return init+((parseFloat(acc?.qt))*(parseFloat(acc?.prix)))},0)
                          +
                          (((facture?.detailFacture[0]?.contenu.
                            filter((d:any)=>{return d.exonereTva=="N"})?.
                              reduce((init:any,acc:any)=>{return init+((parseFloat(acc?.qt))*(parseFloat(acc?.prix)))},0))*16)/100
                            ))
                          }
                ...
                >
                  ...
                  <Text style={{ fontWeight: "bold" }}>
                    {enLettresSansDevise(
                      facture?.detailFacture[0].contenu?.reduce((init:any,acc:any)=>{return init+((parseFloat(acc?.qt))*(parseFloat(acc?.prix)))},0)
                            +
                            (((facture?.detailFacture[0].contenu?.
                            filter((d:any)=>{return d.exonereTva=="N"})?.
                            reduce((init:any,acc:any)=>{return init+((parseFloat(acc?.qt))*(parseFloat(acc?.prix)))},0))*16)/100
...
export default FacturationimportMore;
